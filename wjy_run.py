...
def align_loss(align_input, embedding):
    ...
    l_loss = l_loss * (
                1 - F.one_hot(left, num_classes=node_size) - F.one_hot(right, num_classes=node_size))
...
for turn in range(5):
    for i in trange(epoch):
        ...
        for pairs in [train_pair[i * batch_size:(i + 1) * batch_size] for i in
            ...
            opt.step()
        if i %5==4:
            Lvec, Rvec = get_embedding(dev_pair[:, 0], dev_pair[:, 1], output)
            evaluater.test(Lvec, Rvec)
        new_pair = []
    Lvec, Rvec = get_embedding(rest_set_1, rest_set_2, output)
